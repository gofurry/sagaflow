...
const maxReferenceSize = int64(50 << 20)
...
func (d *Driver) materializeReferences(ctx context.Context, request inference.Request, allowed ...string) (inference.Request, error) {
	...
	for index := range updated.Inputs {
		...
		if _, ok := accepted[input.MediaType]; !ok {
			return inference.Request{}, inference.NewError(inference.ErrorInvalidRequest, provider(request), "unsupported SiliconFlow reference type "+input.MediaType, false, nil)
		}
		if strings.TrimSpace(input.URL) != "" {
			continue
		}
		dataURL, err := inputDataURL(ctx, *input)
		if err != nil {
...
func inputDataURL(ctx context.Context, input inference.Input) (string, error) {
	...
	if info.Size > maxReferenceSize {
		return "", inference.NewError(inference.ErrorInvalidRequest, providerCode, "reference exceeds 50 MB", false, nil)
	}
