...
func TestCreateAndRestoreCompleteWorkspace(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(source.ObjectDir(), "asset.bin"), []byte("asset"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if data, err := os.ReadFile(filepath.Join(target.ObjectDir(), "asset.bin")); err != nil || string(data) != "asset" {
